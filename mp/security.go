...
func ImageSecCheck(filename string) wx.Action {
	return wx.NewUploadAPI(ImageSecCheckURL, url.Values{}, wx.NewUploadForm("media", filename, nil), nil)
}
...
func MediaSecCheckAsync(dest *MediaSecAsyncResult, mediaType SecMediaType, mediaURL string) wx.Action {
	return wx.NewPostAPI(MediaCheckAsyncURL, url.Values{}, func() ([]byte, error) {
...
func MsgSecCheck(content string) wx.Action {
	return wx.NewPostAPI(MsgSecCheckURL, url.Values{}, func() ([]byte, error) {
		return json.Marshal(wx.X{
			"content": content,
		})
	}, nil)
