...
| 模块                   | 功能                                                                          |
| ---------------------- | ----------------------------------------------------------------------------- |
| 微信支付（mch）        | 下单、支付、退款、查询、委托代扣、红包、企业付款、账单、评价数据 等           |
| 微信公众号（offia）    | 网页授权、用户、消息、素材、菜单、发布能力、草稿箱、客服、二维码、事件处理 等 |
| 微信小程序（minip）    | 授权、解密、二维码、消息、素材、插件、URL Scheme、URL Link、OCR、事件处理 等  |
| 企业微信小程序（corp） | 支持几乎全部服务端API                                                         |
